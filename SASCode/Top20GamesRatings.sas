...
PROC SQL;
    CREATE TABLE mydata.top_positive_games AS
    SELECT c.app_id, 
           c.name, 
           INPUT(c.positive, COMMA12.) AS positive_count, /* Convert to numeric */
    ...
    WHERE c.review_score_description = "Overwhelmingly Positive"
    ORDER BY positive_count DESC;
QUIT;

DATA mydata.top_positive_games;
    SET mydata.top_positive_games (OBS=20);
RUN;

DATA mydata.top_positive_games_ratio;
    SET mydata.top_positive_games;
    positive_ratio = positive_count; /* Calculate percentage */
RUN;

PROC SORT DATA=mydata.top_positive_games_ratio;
    BY DESCENDING positive_ratio; /* Ensures highest appears at the top */
RUN;
...
TITLE "Top 20 Overwhelmingly Positive Games";
PROC SGPLOT DATA=mydata.top_positive_games_ratio DATATTRMAP=attrmap;
    HBAR name / RESPONSE=positive_ratio
    ...
    XAXIS LABEL="Number of Positive Reviews" GRID;
